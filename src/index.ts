...
program
  ...
  .option(
    "-f, --force-api",
    "Forçar requisição a API e ignorar arquivo de cache",
    false,
  )
  .addHelpText(
    "after",
...
(async () => {
  ...
  const restaurants = shouldFetch
    ? await fetchAndSaveCache()
    : await readLocalJSON();
  ...
  console.log(
    `Fonte de dados: ${shouldFetch ? "API online" : "Arquivo local"}`,
  );
